...
static int strcmp_hello (char *src, char *dst); 
...
int main() {
    ...
    if (rc == NO_INPUT) {
		...
		//if (strcmp(buff, "Bob") == 0 || strcmp(buff, "Alice") == 0) {
		if (strcmp_hello(buff, "Bob") == 0 || strcmp_hello(buff, "Alice") == 0) {
    		printf ("Hello [%s]\n", buff);
		}
	}
...
int strcmp_hello (char *src, char *dst) {
	int index = 0;
	while(src[index] != '\0' && src[index] == dst[index]) {
		index ++;
	}
